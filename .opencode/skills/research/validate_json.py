...
def extract_json_fields(data, category_mapping=None):
    ...
    nested_keys = {key for aliases in category_mapping.values() for key in aliases}
    fields = set()
    stack = [(data, True)]
...
def validate_json(json_path: Path, all_fields, required_fields, field_categories):
    ...
    return {
        ...
        "missing_by_category": {key: sorted(value) for key, value in missing_by_category.items()},
